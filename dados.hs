...
module Dados (escrever_arquivo, existe_nome, sortear_nome) where
	...
	sortear_nome :: IO String
	sortear_nome = do
				existe_arquivo <- doesFileExist "nomes.txt"
				-- verifica se o arquivo existe
				if existe_arquivo then do
					-- abre o arquivo para leitura
					arq <- openFile "nomes.txt" ReadMode
					-- obtém todo o conteúdo do arquivo
					nomes <- hGetContents arq
					-- obtém uma lista de nomes
					let lista_nomes = lines nomes
					-- sorteia um nome
					num <- randomRIO(1::Int, (length lista_nomes))
					-- retorna o elemento da lista associado ao número sorteado
					return (lista_nomes !! (num - 1))
				else do
					return ""